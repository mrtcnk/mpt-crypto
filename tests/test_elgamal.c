...
static void test_encryption(const secp256k1_context* ctx) {
    ...
    EXPECT(secp256k1_elgamal_encrypt(ctx, &c1, &c2, &pubkey, 12345, blinding_factor) == 1);
    printf("Test passed!\n");
}
...
static void test_encryption_decryption_roundtrip(const secp256k1_context* ctx) {
    ...
    EXPECT(secp256k1_elgamal_generate_keypair(ctx, privkey, &pubkey) == 1);
    EXPECT(secp256k1_elgamal_generate_keypair(ctx, blinding_factor, &temp_pubkey) == 1);
    EXPECT(secp256k1_elgamal_encrypt(ctx, &c1, &c2, &pubkey, original_amount, blinding_factor) == 1);
...
static void test_homomorphic_operations(const secp256k1_context* ctx) {
...

    {
        ...
        EXPECT(secp256k1_elgamal_encrypt(ctx, &a_c1, &a_c2, &pubkey, amount_a, k_a) == 1);
    }
    {
        unsigned char k_b[32];
        secp256k1_pubkey temp_pubkey;
        EXPECT(secp256k1_elgamal_generate_keypair(ctx, k_b, &temp_pubkey) == 1);
        EXPECT(secp256k1_elgamal_encrypt(ctx, &b_c1, &b_c2, &pubkey, amount_b, k_b) == 1);
    }

    EXPECT(secp256k1_elgamal_add(ctx, &result_c1, &result_c2, &a_c1, &a_c2, &b_c1, &b_c2) == 1);
    EXPECT(secp256k1_elgamal_decrypt(ctx, &decrypted_result, &result_c1, &result_c2, privkey) == 1);
    EXPECT(decrypted_result == amount_a + amount_b);

    EXPECT(secp256k1_elgamal_subtract(ctx, &result_c1, &result_c2, &a_c1, &a_c2, &b_c1, &b_c2) == 1);
    EXPECT(secp256k1_elgamal_decrypt(ctx, &decrypted_result, &result_c1, &result_c2, privkey) == 1);
    EXPECT(decrypted_result == amount_a - amount_b);
    printf("Test passed!\n");
}
...
static void test_canonical_zero(const secp256k1_context* ctx) {
    ...
    // Generate it once
    EXPECT(generate_canonical_encrypted_zero(ctx, &c1_a, &c2_a, &pubkey, account_id, issuance_id) == 1);

    // Generate it a second time with the same inputs
    EXPECT(generate_canonical_encrypted_zero(ctx, &c1_b, &c2_b, &pubkey, account_id, issuance_id) == 1);

    // 1. Verify that it decrypts to zero
    EXPECT(secp256k1_elgamal_decrypt(ctx, &decrypted_amount, &c1_a, &c2_a, privkey) == 1);
...
static void test_verify_encryption(const secp256k1_context* ctx) {
    ...
    EXPECT(secp256k1_elgamal_verify_encryption(ctx, &c1, &c2, &pubkey, amount, blinding_factor) == 1);

    /* 2. Test zero-value encryption verification (The special case we added) */
    EXPECT(secp256k1_elgamal_encrypt(ctx, &c1, &c2, &pubkey, zero_amount, blinding_factor) == 1);
    EXPECT(secp256k1_elgamal_verify_encryption(ctx, &c1, &c2, &pubkey, zero_amount, blinding_factor) == 1);

    /* 3. Test detection of incorrect amount */
    EXPECT(secp256k1_elgamal_verify_encryption(ctx, &c1, &c2, &pubkey, amount + 1, blinding_factor) == 0);

    /* 4. Test detection of tampered ciphertext */
    c2 = c1; // Force a mismatch
    EXPECT(secp256k1_elgamal_verify_encryption(ctx, &c1, &c2, &pubkey, zero_amount, blinding_factor) == 0);
